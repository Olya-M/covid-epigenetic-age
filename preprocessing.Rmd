...
setwd('C:/Users/olyam/Documents/ML/Portfolio/p1/Horvath/p2')
```
...
altum_cpgs = fread('AltumAge_cpgs.csv')
...
for (i in 1:length(targets_frame[[1]])){
  ...
  rgs <-subsetByLoci(rgs, includeLoci=unlist(altum_cpgs[-1]))
  betas <- cbind(betas, getBeta(rgs, type="Illumina"))
  print(i)
}

betas_dataframe <- data.frame(betas)
betas_dataframe['V1'] <- rownames(betas_dataframe)
fwrite(betas_dataframe, unprocessed_filename, row.names=F, sep=',') 
```

```{r}
# Downloaded GSE167202_ProcessedBetaValues.txt.gz from https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE167202 - first did normalization with this file to see if re-normalizing could work for age estimation (it didn't)
...
The code and comments in the cells below are from Steve Horvath's "Estimate DNA methylation age with R tutorial" (with some very slight modifications): https://horvath.genetics.ucla.edu/html/dnamage/TUTORIAL1.pdf
...
datClock=read.csv("HorvathClock/AdditionalFile3.csv") # unfortunately there weren't enough cpgs that matched up with the cpgs in the original Horvath clock
...
The cell below won't work with the example dataset (not enough matching cpgs):
```{r}
